...
class Queue:
    ...
    def send(self, name: str, struct: dict):
        ...
        self.redis.hmset(uid, struct)

        self.redis.lpush(name, uid)

    def receive(self, name: str) -> dict:
        task_uid = self.redis.rpop(name)

        if task_uid:
            log.info("job uuid is - {}".format(task_uid))
            data_for_task = self.redis.hgetall(task_uid)
        else:
            data_for_task = {}
